...
export class Orchestrator extends EventEmitter<OrchestratorEventMap> {
  ...
  runTask(agentId: string, taskId: string, prompt: string, opts?: RunTaskOpts): void {
    ...
    if (this.agentManager.isTeamLead(agentId)) {
      teamContext = this.agentManager.getTeamRoster(session.teamId);
  ...
  getTeamRoster(teamId?: string): string {
    return this.agentManager.getTeamRoster(teamId);
  }
      ...
      if (escalation) {
        ...
        if (leadId && leadId !== agentId) {
          ...
          if (leadSession) {
            ...
            const teamContext = this.agentManager.getTeamRoster(leadSession.teamId);
