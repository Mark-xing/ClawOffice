...
export class AgentSession {
  ...
  async runTask(taskId: string, prompt: string, repoPath?: string, teamContext?: string, isUserInitiated = false, phaseOverride?: string) {
    ...
    try {
      ...
      // Apply extra env vars from backend (e.g. custom HOME to skip MCP loading)
      for (const [key, val] of Object.entries(this.backend.extraEnv ?? {})) {
        cleanEnv[key] = val;
      }
      ...
      // Perf optimization: temporarily disable MCP servers for CodeBuddy to avoid 30s+ startup delay.
      // MCP servers like chrome-devtools, figma etc. each take 2-5s to init via npx.
      const mcpPath = path.join(homedir(), ".codebuddy", "mcp.json");
      const mcpBackupPath = mcpPath + ".oo-backup";
      let mcpDisabled = false;
      if (this.backend.command === "codebuddy" && existsSync(mcpPath)) {
        try {
          const { renameSync: renSync } = require("fs");
          renSync(mcpPath, mcpBackupPath);
          writeFileSync(mcpPath, '{"mcpServers":{}}');
          mcpDisabled = true;
        } catch (e) {
          console.warn(`[Agent ${this.name}] Failed to disable MCP: ${e}`);
        }
      }
      const restoreMcp = () => {
        if (mcpDisabled && existsSync(mcpBackupPath)) {
          try {
            const { renameSync: renSync } = require("fs");
            const { unlinkSync: ulSync } = require("fs");
            ulSync(mcpPath);
            renSync(mcpBackupPath, mcpPath);
          } catch { /* ignore */ }
          mcpDisabled = false;
        }
      };
      ...
      // Restore MCP config after spawn (child process has already read it)
      setTimeout(restoreMcp, 3000);
      ...
      // Delegation detection regex — supports Latin, CJK, and other Unicode names
      const DELEGATION_RE = /^\s*(?:[-*>]\s*)?(?:\*\*)?@([^\s:]+)(?:\*\*)?:\s*(.+)$/;
      ...
      this.process.on("close", (code) => {
        restoreMcp(); // Ensure MCP config is restored
