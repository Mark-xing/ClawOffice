...
function persistTeamState() {
  ...
  const teams: Array<NonNullable<TeamState["team"]>> = [];
  ...
  // Legacy compat: also set single team field
  if (teams.length > 0) team = teams[0];

  saveTeamState({ agents, team, teams });
}
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        // Check both skill.md (OpenOffice convention) and SKILL.md (ClawHub convention)
...
function getSkillEntryPath(skillName: string): string | null {
  ...
  for (const candidate of ["skill.md", "SKILL.md"]) {
    const p = path.join(SKILLS_DIR, skillName, candidate);
...
// Team-level custom working directories (per teamId, overrides defaultWorkspace for project creation)
const teamWorkDirs = new Map<string, string | undefined>();
...
function handleCommand(parsed: Command | { type: string; [key: string]: any }, meta: CommandMeta) {
  // RBAC enforcement
  ...
  switch (parsed.type) {
    ...
    case "UPDATE_AGENT_SKILLS": {
      const { agentId, skillFiles: newSkillFiles } = parsed;
      const agent = orc.getAgent(agentId);
      if (!agent) {
        console.warn(`[Gateway] UPDATE_AGENT_SKILLS rejected: agent "${agentId}" not found`);
        break;
      }
      console.log(`[Gateway] Updating skills for ${agent.name}: [${(newSkillFiles ?? []).join(", ")}]`);

      // Build new personality with skills injected
      // Strip old skills section if present
      let basePersonality = agent.personality ?? "";
      const skillMarker = "\n\n===== AGENT SKILLS =====\n";
      const markerIdx = basePersonality.indexOf(skillMarker);
      if (markerIdx >= 0) {
        basePersonality = basePersonality.slice(0, markerIdx);
      }

      // Inject new skills
      if (newSkillFiles && newSkillFiles.length > 0) {
        const skillContents: string[] = [];
        for (const skillName of newSkillFiles) {
          const entryPath = getSkillEntryPath(skillName);
          if (entryPath) {
            try {
              const content = readFileSync(entryPath, "utf-8");
        ...
        if (skillContents.length > 0) {
          basePersonality += skillMarker + skillContents.join("\n\n---\n\n");
        }
      }

      // Re-create agent with new personality (preserving all other state)
      const savedWorkDir = agentWorkDirs.get(agentId);
      const wasTeamLead = orc.isTeamLead(agentId);
      const savedAutoMerge = agent.autoMerge;
      if (agent.pid) scanner?.addGracePid(agent.pid);
      orc.removeAgent(agentId);
      ...
      if (wasTeamLead) orc.setTeamLead(agentId);
      if (savedAutoMerge !== undefined) orc.setAgentAutoMerge(agentId, savedAutoMerge);
      if (savedWorkDir) agentWorkDirs.set(agentId, savedWorkDir);

      // Broadcast updated agent to UI
      publishEvent({
        type: "AGENT_SKILLS_UPDATED",
        agentId,
        skillFiles: newSkillFiles ?? [],
      } as any);
      persistTeamState();
      console.log(`[Gateway] Skills updated for ${agent.name}, agent re-created`);
      break;
    }
    case "RUN_TASK": {
    ...
    case "CREATE_TEAM": {
      ...
      const customNames: Record<string, string> = (parsed as any).customNames ?? {};
      const allIds = [leadId, ...memberIds.filter(id => id !== leadId)];
      console.log(`[Gateway] Creating team: lead=${leadId}, members=${memberIds.join(",")}${parsed.workDir ? ` workDir=${parsed.workDir}` : ""}${Object.keys(customNames).length > 0 ? ` customNames=${JSON.stringify(customNames)}` : ""}`);

      // Store team-level working directory override (per teamId)
      const teamId = `team-${nanoid(6)}`;
      teamWorkDirs.set(teamId, parsed.workDir || undefined);
      ...
      for (const defId of allIds) {
        ...
        // Use custom name if provided, then def.name if short, else auto-assign
        let agentName = customNames[defId]?.trim() || def.name;
        if (!customNames[defId]?.trim()) {
    ...
    case "FIRE_TEAM": {
      const targetTeamId = (parsed as any).teamId as string | undefined;
      console.log(`[Gateway] Firing team${targetTeamId ? ` ${targetTeamId}` : " (all)"}`);
      // Record managed PIDs before they're killed
      for (const agent of orc.getAllAgents()) {
        if (targetTeamId && agent.teamId !== targetTeamId) continue;
        const pid = agent.pid;
        if (pid) scanner?.addGracePid(pid);
      }
      if (targetTeamId) {
        // Fire only the specified team
        const teamAgents = orc.getAllAgents().filter(a => a.teamId === targetTeamId);
        for (const agent of teamAgents) {
          orc.removeAgent(agent.agentId);
        }
        orc.clearTeamPhase(targetTeamId);
        teamWorkDirs.delete(targetTeamId);
      } else {
        // Legacy: fire all teams
        orc.fireTeam();
    ...
    case "APPROVE_PLAN": {
      ...
      const agentInfo = orc.getAgent(agentId);
      const thisTeamWorkDir = agentInfo?.teamId ? teamWorkDirs.get(agentInfo.teamId) : undefined;
      console.log(`[Gateway] APPROVE_PLAN: agent=${agentId}${thisTeamWorkDir ? ` teamWorkDir=${thisTeamWorkDir}` : ""}`);
      ...
      const workspace = thisTeamWorkDir || config.defaultWorkspace;
    ...
    case "PING": {
      ...
      console.log(`[Gateway] SKILL_LIST: ${allSkills.length} skills found:`, allSkills.map(s => s.name));
      publishEvent({ type: "SKILL_LIST", skills: allSkills });
      // Broadcast MCP config (from ~/.codebuddy/mcp.json if exists)
      try {
        const mcpPath = path.join(os.homedir(), ".codebuddy", "mcp.json");
        if (existsSync(mcpPath)) {
          const mcpRaw = JSON.parse(readFileSync(mcpPath, "utf-8"));
          publishEvent({ type: "MCP_CONFIG", servers: mcpRaw.mcpServers ?? mcpRaw.servers ?? mcpRaw ?? {} });
        }
      } catch { /* ignore */ }
      break;
    }
    ...
    case "INSTALL_CLAWHUB_SKILL": {
      ...
      if (!slug) break;
      console.log(`[Gateway] Installing ClawHub skill: ${slug}`);
      // Ensure skills directory exists
      if (!existsSync(SKILLS_DIR)) mkdirSync(SKILLS_DIR, { recursive: true });
      // Resolve clawhub binary path
      let clawhubBin = "clawhub";
      try {
        clawhubBin = execSync("which clawhub", { encoding: "utf-8", timeout: 3000 }).trim() || "clawhub";
      } catch { /* fallback */ }

      // clawhub install only supports --force (no --workdir/--dir)
      // It installs to ~/.openclaw/workspace/skills/<slug>/ by default
      const installCmd = `${clawhubBin} install ${slug} --force`;
      console.log(`[Gateway] Running: ${installCmd}`);
      exec(installCmd, { timeout: 30000 }, (err, stdout, stderr) => {
        if (err) {
          console.error(`[Gateway] ClawHub install failed: ${err.message}`);
          console.error(`[Gateway] stderr: ${stderr}`);
          publishEvent({ type: "CLAWHUB_INSTALL_RESULT", slug, success: false, error: err.message } as any);
          return;
        }
        console.log(`[Gateway] ClawHub install output: ${stdout}`);

        // clawhub installs to ~/.openclaw/workspace/skills/<slug>/
        // Copy it to our SKILLS_DIR if it's not already there
        const clawhubSkillDir = path.join(os.homedir(), ".openclaw", "workspace", "skills", slug);
        const targetDir = path.join(SKILLS_DIR, slug);
        if (existsSync(clawhubSkillDir) && clawhubSkillDir !== targetDir) {
          try {
            if (!existsSync(targetDir)) mkdirSync(targetDir, { recursive: true });
            const files = readdirSync(clawhubSkillDir);
            for (const file of files) {
              const src = path.join(clawhubSkillDir, file);
              const dst = path.join(targetDir, file);
              writeFileSync(dst, readFileSync(src));
            }
            console.log(`[Gateway] Copied ${files.length} files from ${clawhubSkillDir} to ${targetDir}`);
          } catch (copyErr) {
            console.error(`[Gateway] Failed to copy skill files: ${copyErr}`);
          }
        }

        // Refresh skill list and broadcast
        const skills = listSkills();
        console.log(`[Gateway] After install, skills: ${skills.map(s => s.name).join(", ")}`);
        publishEvent({ type: "SKILL_LIST", skills });
        publishEvent({ type: "CLAWHUB_INSTALL_RESULT", slug, success: true, skillName: slug } as any);
      });
      break;
    }
    case "SEARCH_CLAWHUB_SKILLS": {
      ...
      if (!query) break;
      console.log(`[Gateway] Searching ClawHub skills: "${query}"`);
      let clawhubBin = "clawhub";
      try {
        clawhubBin = execSync("which clawhub", { encoding: "utf-8", timeout: 3000 }).trim() || "clawhub";
      } catch { /* fallback */ }

      exec(`${clawhubBin} search "${query.replace(/"/g, '\\"')}"`, { timeout: 15000 }, (err, stdout) => {
        if (err) {
          console.error(`[Gateway] ClawHub search failed: ${err.message}`);
          publishEvent({ type: "CLAWHUB_SEARCH_RESULT", query, results: [] } as any);
          return;
        }
        // Parse output: each line is "slug  title  (score)"
        ...
        for (const line of stdout.split("\n")) {
          ...
          if (match) {
            results.push({ slug: match[1], title: match[2].trim(), score: parseFloat(match[3]) });
          }
        }
        console.log(`[Gateway] ClawHub search: ${results.length} results for "${query}"`);
        publishEvent({ type: "CLAWHUB_SEARCH_RESULT", query, results } as any);
      });
      break;
    }
...
async function main() {
  ...
  if (savedState.agents.length > 0) {
    ...
    // Restore team phases — support both new `teams` array and legacy `team` single value
    const teamsToRestore = savedState.teams?.length
      ? savedState.teams
      : (savedState.team ? [savedState.team] : []);
    for (const t of teamsToRestore) {
      if (!orc.getAgent(t.leadAgentId)) continue;
      if (t.phase === "execute") {
