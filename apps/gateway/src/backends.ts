...
const backends: AIBackend[] = [
  ...
  },
  {
    ...
    command: "codebuddy",
    supportsStdin: true,
    instructionPath: ".codebuddy/CODEBUDDY.md",
    stability: "stable",
    guardType: "hooks",
    supportsResume: true,
    ...
    buildArgs(prompt, opts) {
      ...
      args.push("--append-system-prompt", "请始终使用中文回复用户。所有输出、解释、注释都用中文，代码中的变量名和关键字保持英文。");
      // Disable MCP servers to avoid 12+ server init overhead (~20s)
