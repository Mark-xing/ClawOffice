...
const AgentProfileModal = dynamic(() => import("@/components/office/ui/AgentProfileModal"), { ssr: false });
...
export default function OfficePage() {
  ...
  const [editingTeamTab, setEditingTeamTab] = useState<string | null>(null);
  const teamTabInputRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("open-office-team-names");
      if (saved) setTeamNames(JSON.parse(saved));
    } catch { /* ignore */ }
  }, []);
  useEffect(() => {
    if (Object.keys(teamNames).length > 0) {
      localStorage.setItem("open-office-team-names", JSON.stringify(teamNames));
    }
  }, [teamNames]);
  ...
  const handleCreateTeam = useCallback((leadId: string, memberIds: string[], backends: Record<string, string>, workDir?: string, customNames?: Record<string, string>) => {
    sendCommand({ type: "CREATE_TEAM", leadId, memberIds, backends, workDir, customNames });
    setShowHireTeamModal(false);
    setExpandedSection("team");
    // New teamId will be auto-selected when agents appear (via useEffect on teamIds)
    setSelectedTeamId(null); // Reset so useEffect picks the newest team
  ...
  const hasTeam = useMemo(() => {
    for (const a of agents.values()) {
      if (a.teamId && (!selectedTeamId || a.teamId === selectedTeamId)) return true;
    }
    return false;
  ...
  const teamBusy = useMemo(() => {
    for (const a of agents.values()) {
      if (a.teamId && (!selectedTeamId || a.teamId === selectedTeamId)) {
        if (a.status === "working" || a.status === "waiting_approval") return true;
      }
    }
    return false;
  }, [agents, selectedTeamId]);
  ...
  const handleStopTeam = useCallback(() => {
    ...
    // Stop only agents of the currently selected team
    const targetAgents = selectedTeamId
      ? Array.from(agents.values()).filter((a) => a.teamId === selectedTeamId)
      : Array.from(agents.values()).filter((a) => !!a.teamId);
  ...
  const handleFireTeam = useCallback(async () => {
    // Fire only the currently selected team, or all teams if none selected
    const targetAgents = selectedTeamId
      ? Array.from(agents.values()).filter((a) => a.teamId === selectedTeamId)
      : Array.from(agents.values()).filter((a) => !!a.teamId);
    if (targetAgents.length === 0) return;
    const teamIds = new Set(targetAgents.map(a => a.teamId).filter(Boolean));
    const msg = `Fire this team (${targetAgents.length} agents)?`;
    if (!await confirm(msg)) return;
    for (const tid of teamIds) {
      sendCommand({ type: "FIRE_TEAM", teamId: tid });
    }
    for (const a of targetAgents) {
    ...
    // Reset selectedTeamId so it picks the next team
    setSelectedTeamId(null);
  }, [agents, clearTeamMessages, confirm, selectedTeamId]);
  ...
  // Group team agents by teamId for multi-team support
  const teamGroups = useMemo(() => {
    const groups = new Map<string, typeof teamAgents>();
    for (const a of teamAgents) {
      if (!a.teamId) continue;
      const list = groups.get(a.teamId) ?? [];
      list.push(a);
      groups.set(a.teamId, list);
    }
    return groups;
  }, [teamAgents]);
  const teamIds = useMemo(() => Array.from(teamGroups.keys()), [teamGroups]);

  // Auto-select first team if none selected or selected team was dissolved
  useEffect(() => {
    if (expandedSection === "team") {
      if (!selectedTeamId || !teamGroups.has(selectedTeamId)) {
        if (teamIds.length > 0) setSelectedTeamId(teamIds[0]);
      }
    }
  }, [expandedSection, selectedTeamId, teamIds, teamGroups]);

  // Agents for the currently selected team
  const activeTeamAgents = selectedTeamId ? (teamGroups.get(selectedTeamId) ?? []) : teamAgents;
  ...
  const activeAgentIds = (expandedSection === "agents" ? soloAgents
    : expandedSection === "team" ? activeTeamAgents
    ...
    <div style={{ height: "100vh", width: "100vw", position: "relative", overflow: "hidden", display: "flex", backgroundColor: TERM_BG }}>
      ...
      {!isMobile && <>
        ...
        <div className={`term-dotgrid${scrollFrozen ? " console-transitioning" : ""}`} style={{
          ...
          {/* Bookmark tabs */}
          {[
            { key: "agents" as const, label: "Agents", teamId: null },
            // Generate one tab per team (instead of a single "Team" tab)
            ...(teamIds.length > 0
              ? teamIds.map((tid, i) => {
                  // Use custom name if set, otherwise leader name or default
                  const teamGroup = teamGroups.get(tid) ?? [];
                  const lead = teamGroup.find(a => agents.get(a.agentId)?.isTeamLead);
                  const defaultLabel = teamIds.length === 1
                    ? "Team"
                    : (lead ? lead.name : `T${i + 1}`);
                  const label = teamNames[tid] || defaultLabel;
                  return { key: "team" as const, label, teamId: tid };
                })
              : [{ key: "team" as const, label: "Team", teamId: null }]
            ),
            { key: "external" as const, label: "Ext", teamId: null },
          ].map((tab) => {
            const active = expandedSection === tab.key && (tab.key !== "team" || selectedTeamId === tab.teamId);
            const accentColor = TERM_GREEN;
            const isEditing = editingTeamTab === tab.teamId && tab.teamId !== null;
            return (
              <button
                key={tab.teamId ?? tab.key}
                onClick={() => {
                  if (isEditing) return; // Don't switch while editing
                  setExpandedSection(tab.key);
                  if (tab.key === "team" && tab.teamId) {
                    setSelectedTeamId(tab.teamId);
                    const teamGroup = teamGroups.get(tab.teamId) ?? [];
                    if (teamGroup.length > 0) { setSelectedAgent(teamGroup[0].agentId); setChatOpen(true); }
                  } else {
                    const list = tab.key === "agents" ? soloAgents : tab.key === "team" ? activeTeamAgents : externalAgents;
                    if (list.length > 0) { setSelectedAgent(list[0].agentId); setChatOpen(true); }
                  }
                }}
                onDoubleClick={() => {
                  // Double-click to rename team tab
                  if (tab.key === "team" && tab.teamId) {
                    setEditingTeamTab(tab.teamId);
                    setTimeout(() => teamTabInputRef.current?.focus(), 50);
                  }
                }}
                style={{
                  ...
                  padding: "0 5px",
                  height: isEditing ? 90 : 72,
                  border: "none", cursor: tab.key === "team" && tab.teamId ? "pointer" : "pointer",
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        ...
                        if (val && tab.teamId) {
                          setTeamNames(prev => ({ ...prev, [tab.teamId!]: val }));
                        }
                        setEditingTeamTab(null);
      ...
      {profileAgentId && (() => {
        const agent = agents.get(profileAgentId);
        if (!agent) return null;
        const matchedDef = agentDefs.find(d => d.name === agent.name || d.role === agent.role?.split(" — ")[0]);
        return (
          <AgentProfileModal
            ...
            agentDef={matchedDef ?? null}
            availableSkills={availableSkills}
            mcpServers={mcpServers}
            sendCommand={sendCommand}
            assetsReady={assetsReady}
            onClose={() => { setProfileAgentId(null); setChatOpen(true); }}
          />
        );
      })()}
