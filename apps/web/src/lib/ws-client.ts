...
function doConnect() {
  ...
  socket.onmessage = (evt) => {
    try {
      ...
      // Try Zod schema first; fall back to raw JSON for extension events
      // (CLAWHUB_SEARCH_RESULT, CLAWHUB_INSTALL_RESULT, MCP_CONFIG, AGENT_SKILLS_UPDATED etc.)
      let event: any;
      const result = GatewayEventSchema.safeParse(msg);
      if (result.success) {
        event = result.data;
      } else {
