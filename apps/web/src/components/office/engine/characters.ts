...
export function updateCharacter(
  ...
  switch (ch.state) {
    ...
    case CharacterState.TALK: {
      // Stand facing the conversation partner, use idle/standing frame.
      // Exit is managed by officeState.update() to keep both partners in sync.
      ch.frame = 0
      if (ch.talkTimer > 0) {
        ch.talkTimer -= dt
        if (ch.talkTimer < 0) ch.talkTimer = 0
      }
      break
    }
    ...
    case CharacterState.WALK: {
      ...
      if (ch.path.length === 0) {
        ...
        // Priority: if walking to a conversation partner, enter TALK immediately
        if (ch.talkTarget !== null) {
      ...
      // Don't redirect to seat if walking to a conversation partner
      if (ch.isActive && ch.seatId && ch.talkTarget === null) {
        const seat = seats.get(ch.seatId)
        if (seat) {
