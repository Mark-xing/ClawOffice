...
export function useSceneBridge(
  ...
  useEffect(() => {
    ...
    const unsub = useOfficeStore.subscribe((state) => {
      ...
      // Team chat messages -> speech bubble on sender + face-to-face conversation
      ...
      if (state.teamMessages.length > prevTeamCount) {
        for (let i = prevTeamCount; i < state.teamMessages.length; i++) {
          ...
          // Trigger face-to-face conversation when one agent talks to another
          if (tm.toAgentId && tm.fromAgentId !== tm.toAgentId && adapter.startConversation) {
            adapter.startConversation(tm.fromAgentId, tm.toAgentId);
          }
        }
      }
      prevTeamMsgCountRef.current = state.teamMessages.length;
