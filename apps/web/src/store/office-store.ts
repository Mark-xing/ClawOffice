...
interface OfficeStore {
  ...
  clawhubSearchResults: Array<{ slug: string; title: string; score: number }>;
  ...
  handleEvent: (event) => {
    set((state) => {
      ...
      switch (event.type) {
        ...
        case "MCP_CONFIG": {
          return { agents, mcpServers: (event as any).servers ?? {} };
        }
        case "CLAWHUB_SEARCH_RESULT": {
          return { agents, clawhubSearchResults: (event as any).results ?? [] };
        }
        case "CLAWHUB_INSTALL_RESULT": {
          // Just ignore, SKILL_LIST update handles the UI refresh
          return { agents };
        }
        case "CONFIG_LOADED": {
